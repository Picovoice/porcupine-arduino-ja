# Porcupine SDK for Arduino boards - Japanese language
...
3. Search for `Porcupine_JA`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Porcupine_JA`->`PorcupineExample`.
...
1. Compile and upload the `Porcupine_JA/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
